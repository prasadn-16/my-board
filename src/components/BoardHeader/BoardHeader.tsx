...
const BoardHeader = ({ boardId, title, createdBy }: BoardHeaderProps) => {
  ...
  const canDelete = isAdmin || (createdBy && user?.email === createdBy);
      ...
      {isEditing ? (
        <div className="flex gap-2 mb-2 w-full">
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" &&
              dispatch(saveBoardTitle({ boardId, title: editingTitle }))
            }
